...
export default {
  ...
  draw(g, rows, [x0, y0, x1, y1], ctx) {
    const { S, th, paint: P } = ctx, focus = ctx.options.focus || "world", keyH = S * 0.05;
    ...
    const dr = Math.max(0.8, S * (ctx.options.dot || 0.0028)), keyText = `one dot = ${core.num(per, ctx)}`;
    const room = geo.mapRoom(focus, [x0, y0, x1, y1], ctx, { bottom: [dr * 6 + S * 0.01 + core.measure(keyText, "arvo", S * 0.021).w, keyH] });
    const base = geo.basemap(g, focus, [x0, y0, x1, y1], ctx, { fit: room.fit, terrain: ctx.options.terrain ?? false });
    ...
    const [kx, ky] = [room.bottom[0], room.bottom[1] + keyH / 2];
    core.dot(key, kx + dr * 3, ky, dr * 2, th.strong);
    core.text(key, keyText, { x: kx + dr * 6 + S * 0.01, y: ky, face: "arvo", px: S * 0.021, fill: th.body, valign: "fmid" });
    ...
    if (hot >= 0) {
      ...
      geo.labelPoints(g.append("g").attr("id", "labels"), [{ xy: c, r: S * 0.02, text: `${r.label} ${core.num(r.value, ctx)}`, face: "arvoBold", px: S * 0.024, fill: P.words(hot) }], [x0, y0, x1, y1], ctx);
    }
    return null;
  },
};

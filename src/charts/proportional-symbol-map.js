...
export default {
  ...
  draw(g, rows, [x0, y0, x1, y1], ctx) {
    ...
    geo.labelPoints(g.append("g").attr("id", "labels"), named.map((p) => ({ xy: p.xy, r: R(p.r.value), text: `${p.r.label} ${core.num(p.r.value, ctx)}`,
      face: P.on(p.i) ? "arvoBold" : "arvo", px: S * (P.on(p.i) ? 0.024 : 0.020), fill: P.on(p.i) ? P.words(p.i) : th.ink })), [x0, y0, x1, y1 - keyH], ctx);
    // the scale: the biggest value and a quarter of it
    const key = g.append("g").attr("id", "key"), ky = y1 - keyH / 2;
    let kx = x0;
    for (const v of [vmax, vmax / 4]) {
      const rr = Math.min(R(v), keyH * 0.42);
